...
```{r boxplot, echo = F, fig.align = "center", cache = T, fig.cap = "Box plot"}
...
```{r pairs, echo = F, fig.align = "center", cache = T, fig.cap = "Pair plot"}
#pairs(select(mtcars, !c(disp,drat,vs,gear,carb)))
...
df <- data.frame('qsec' = round(cor(mtcars$wt,mtcars$qsec),2), 
                 'hp' = round(cor(mtcars$wt,mtcars$hp),2),
                 row.names = 'wt')
df
```
...
```{r diagPlot, echo = F, fig.align = "center", cache = T, fig.cap = "Diagnostic plotting"}
par(mfrow = c(1,2))
...
```{r scatterplot, echo = F, fig.align = "center", cache = T, ref.label = "scatterplot", fig.cap = "4-dim scatterplot"}
