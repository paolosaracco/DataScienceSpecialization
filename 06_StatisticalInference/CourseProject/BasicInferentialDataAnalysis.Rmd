...
c("grp1" = colMeans(dose.5), "grp2" = colMeans(dose1), "grp3" = colMeans(dose2))
```
...
```{r confInt, echo = F}
grp1 <- t.test(dose.5,alternative = "two.sided")$conf.int[1:2]
grp2 <- t.test(dose1,alternative = "two.sided")$conf.int[1:2]
grp3 <- t.test(dose2,alternative = "two.sided")$conf.int[1:2]

df <- as.data.frame(rbind(grp1,grp2,grp3))
names(df) <- c("min","max")
df
```
